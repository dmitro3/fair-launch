...
export const AdminSetup = () => {
    ...
    const handleRevokeMintChange = (value: boolean) => {
        updateAdminSetup({ revokeMintAuthority: value });
    };

    const handleRevokeFreezeChange = (value: boolean) => {
        updateAdminSetup({ revokeFreezeAuthority: value });
    };
    ...
    const handleAdminStructureChange = (value: string) => {
                ...
                <div className="space-y-6 mt-6">
                    <div>
                        <div className="flex flex-col items-start mb-2 border border-gray-200 rounded-lg">
                            <div className={`flex items-center justify-between w-full p-3  ${adminSetup.revokeMintAuthority ? 'bg-gray-50 border-b border-gray-200 rounded-t-lg' : ''}`}>
                                <div>
                                ...
                                <button
                                    type="button"
                                    onClick={() => handleRevokeMintChange(!adminSetup.revokeMintAuthority)}
                                    className={`relative inline-flex h-6 w-12 items-center rounded-full transition-colors duration-200 ease-in-out ${adminSetup.revokeMintAuthority ? 'bg-black' : 'bg-gray-200'}`}
                                >
                                    <span
                                    className={`inline-block h-5 w-5 transform rounded-full bg-white transition duration-200 ease-in-out ${adminSetup.revokeMintAuthority ? 'translate-x-6' : 'translate-x-1'}`}
                                    />
                                </button>
                            </div>
                            {
                                    ...
                                    <div className='flex flex-col gap-2 p-3 w-full'>
                                        <RadioGroup defaultValue="comfortable" className='w-full'>
                                            ...
                                            <div className='flex flex-col gap-1 w-full'>
                                                ...
                                                <Input placeholder='Mint Authority Wallet Address' className='ml-5 mt-1 text-xs placeholder:text-xs w-[90%]'/>
                                            </div>
                                        </RadioGroup>
                                    </div>
                                )
                        ...
                        <div className="flex flex-col items-start mb-2 border border-gray-200 rounded-lg">
                            <div className={`flex items-center justify-between w-full p-3  ${adminSetup.revokeFreezeAuthority ? 'bg-gray-50 border-b border-gray-200 rounded-t-lg' : ''}`}>
                                <div>
                                ...
                                <button
                                    type="button"
                                    onClick={() => handleRevokeFreezeChange(!adminSetup.revokeFreezeAuthority)}
                                    className={`relative inline-flex h-6 w-12 items-center rounded-full transition-colors duration-200 ease-in-out ${adminSetup.revokeFreezeAuthority ? 'bg-black' : 'bg-gray-200'}`}
                                >
                                    <span
                                    className={`inline-block h-5 w-5 transform rounded-full bg-white transition duration-200 ease-in-out ${adminSetup.revokeFreezeAuthority ? 'translate-x-6' : 'translate-x-1'}`}
                                    />
                                </button>
                            </div>
                            {
                    ...
                    <div>
                        ...
                        {dropdownOpen && (
                            ...
                            {adminStructures.map((structure) => (
                                ...
                                onClick={() => handleAdminStructureChange(structure.value)}
                                >
                                {structure.label}
                                </div>
                            ))}
                    ...
                    {adminSetup.adminStructure === 'multisig' && (
                        <div className="bg-gray-50 rounded-xl border border-gray-100 p-6 mt-4">
                            ...
                            <div className="mb-5">
                                ...
                                <div className="relative mt-1">
                                    <select
                                        ...
                                        disabled
                                    >
                                        <option>Multi-Signature</option>
                                    </select>
                                </div>
                                <div className="text-sm text-gray-500 mt-1">How token management decisions will be made</div>
                            </div>
                            <div className="mb-2">
                                <Label className="font-medium">Required Signatures: {requiredSignatures}</Label>
                                <SliderCustom
                                    min={1}
                                    max={10}
                                    value={[requiredSignatures]}
                                    onValueChange={([val]: number[]) => setRequiredSignatures(val)}
                                    className="mt-2"
                                />
